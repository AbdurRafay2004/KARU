...
export function ProductListingPage() {
    ...
    // Using cast for artisanIds because selectedArtisans is string[] but API expects Id<"artisans">[]
    ...
    // Filter and sort products (Client side filtering removed for category/artisan, kept for sort and search-price)
    const filteredProducts = useMemo(() => {
        if (!products) return [];

        let filtered = products;

        // Search results don't support price filtering at database level in the search query directly (it takes a query string),
        // so client-side filter if search is active
        if (searchQuery) {
            filtered = filtered.filter(p => p.price >= priceRange[0] && p.price <= priceRange[1]);
             // Note: search results might not respect categories/artisans filters if the search API doesn't support them.
             // But for now, we assume search overrides specific filters or the user clears filters when searching.
        }
